...
you should always request the minimum permissions necessary for your extension's functionality. If you only need to block requests to specific domains, specify those domains in your host permissions rather than using `<all_urls>`. This focused approach leads to better user trust and a smoother review process for the Chrome Web Store.
...
With this knowledge, you're now equipped to build sophisticated network filtering extensions that use the full power of Manifest V3. Start experimenting with your own rule sets, and don't hesitate to explore the additional possibilities that the DeclarativeNetRequest API offers.
