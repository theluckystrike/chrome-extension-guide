...
This language flexibility means you can leverage existing libraries and codebases that were never designed for the web. For example, you could port a mature Rust cryptographic library to WebAssembly and use it in your extension without rewriting the entire implementation in JavaScript.
...
The crate-type "cdylib" produces a C-compatible dynamic library that JavaScript can load. The wasm-bindgen crate provides the bindings that allow JavaScript and Rust to communicate seamlessly.
