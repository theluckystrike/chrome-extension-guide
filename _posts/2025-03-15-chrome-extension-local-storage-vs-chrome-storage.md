...
chrome.storage is a storage API specifically designed for Chrome extensions. It provides a more solid and feature-rich alternative to localStorage, addressing many of the limitations that make localStorage less suitable for extension development. Introduced as part of the Chrome extension platform, chrome.storage offers capabilities that align with the unique requirements of browser extensions.

The API is asynchronous by design, which means it doesn't block the main thread during read and write operations. This asynchronous nature is particularly important for extensions, where UI responsiveness is crucial for providing a positive user experience. The API integrates smoothly with Chrome's extension architecture and provides additional features like storage quota management, change listeners, and built-in synchronization.
...
Extensions that prioritize UI responsiveness should use chrome.storage's asynchronous API. Popup windows and background scripts that perform frequent storage operations will remain fluid and responsive with chrome.storage, whereas localStorage could cause noticeable lag during intensive operations.
