...
Understanding the complete update lifecycle helps you build robust extensions that handle updates gracefully. Chrome provides several events and methods that let you respond to different stages of the update process.
...
The chrome.runtime.onInstalled event fires when your extension is first installed, when it is updated to a new version, or when the extension is being重新安装（reinstalled）。This is your opportunity to perform initialization tasks or migrations when a new version takes effect.
