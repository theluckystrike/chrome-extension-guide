...
async function startFocus() {
  ...
  chrome.alarms.create('pomodoro-tick', { periodInMinutes: 0.5 }); // Minimum alarm interval is 30 seconds
  chrome.alarms.create('pomodoro-phase', { delayInMinutes: data.pomodoroWork });
  chrome.alarms.create('stats-update', { periodInMinutes: 1 });
...
async function handlePhaseEnd() {
  ...
  if (currentPhase === 'work') {
    ...
    chrome.alarms.create('pomodoro-phase', { delayInMinutes: data.pomodoroBreak });
    const existing = await chrome.declarativeNetRequest.getDynamicRules();
...
onMessage('take-break', async () => {
  ...
  await updateBlockRules();
  chrome.alarms.create('break-end', { delayInMinutes: 5 });
  return { ok: true };
});
...
`chrome.alarms` is the correct MV3 timer mechanism. Service workers can be terminated at any time, making `setTimeout`/`setInterval` unreliable. The `pomodoro-tick` alarm keeps the badge current. The popup polls `get-timer` every second for its own display.
