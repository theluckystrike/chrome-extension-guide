...
async function scheduleAlarm(): Promise<void> {
  ...
  await [chrome.alarms](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).clear(ALARM_NAME);
  [chrome.alarms](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).create(ALARM_NAME, {
    delayInMinutes: 1,
...
function notifyPriceDrop(product: TrackedProduct, newPrice: number, dropPercent: number): void {
  const sym = product.currency === "USD" ? "$" : product.currency + " ";
  [chrome.notifications](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).create(`price-drop-${product.id}`, {
    type: "basic",
...
function notifyTargetReached(product: TrackedProduct, price: number): void {
  const sym = product.currency === "USD" ? "$" : product.currency + " ";
  [chrome.notifications](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).create(`target-${product.id}`, {
    type: "basic",
    iconUrl: "../icons/icon128.png",
...
[chrome.notifications](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).onButtonClicked.addListener((id, buttonIndex) => {
  if (buttonIndex === 0) {
    const productId = id.replace(/^(price-drop-|target-)/, "");
    productsStorage.get().then((products) => {
      const p = products.find((x) => x.id === productId);
      if (p) [chrome.tabs](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).create({ url: p.url });
    });
  }
  [chrome.notifications](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).clear(id);
});

[chrome.alarms](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).onAlarm.addListener((alarm) => {
...
async function loadProducts(): Promise<void> {
  ...
  document.querySelectorAll("[data-action='open']").forEach((btn) =>
    btn.addEventListener("click", () => [chrome.tabs](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).create({ url: (btn as HTMLElement).dataset.url! })));
...
async function render(): Promise<void> {
  ...
  document.querySelectorAll("[data-url]").forEach((b) =>
    b.addEventListener("click", () => [chrome.tabs](https://bestchromeextensions.com/extension-monetization-playbook/monetization/api-monetization).create({ url: (b as HTMLElement).dataset.url! })));
