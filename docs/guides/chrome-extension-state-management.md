...
1. **Persistent storage**: chrome.storage.local, chrome.storage.sync, or IndexedDB
...
## Pattern 1: Single Source of Truth with chrome.storage

The most reliable pattern is to treat `chrome.storage.local` as your single source of truth. All contexts read from and write to storage, ensuring consistency across the extension.
...
const saveSettings = debounce(async (newSettings) => {
  await chrome.storage.local.set({ myExtension_settings: newSettings });
...
setInterval(() => {
  chrome.storage.local.get('heartbeat').then(result => {
...
document.getElementById('login').addEventListener('click', async () => {
  userData = await fetchUserData();
  await chrome.storage.local.set({ myExtension_user: userData });
});
```
...
function updateUI(data) {
  chrome.storage.local.set({ myExtension_data: processData(data) });
}
...
async function saveLargeData(data) {
  ...
  await chrome.storage.local.set({ myExtension_largeData: data });
  return true;
}
```
...
async function updateCounter() {
  ...
  await chrome.storage.local.set({ counter: count + 1 });
}
...
Key takeaways:
1. Always persist state to chrome.storage or IndexedDB
