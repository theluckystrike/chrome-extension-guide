...
WXT is built on top of Vite and takes a modern, opinionated approach to Chrome extension development. It was created by the team behind VueUse and draws heavily from the Nuxt.js philosophy, providing a zero-config starting point while allowing full customization.
...
CRXJS (Chrome Extension JavaScript) takes a different approach, it's primarily a build tool rather than a full framework. Built by Drew B, CRXJS focuses on making the development server and build process as smooth as possible.
...
Plasmo offers solid HMR support across all extension contexts. The framework uses a sophisticated injection system that updates modified components in real-time. Popup, options, and side panel changes reflect immediately, while content scripts benefit from automatic page refresh triggers.
...
Winner: WXT and Plasmo tie for the best HMR experience. Both provide smooth, automatic updates across all extension contexts. CRXJS is slightly behind but still offers a solid experience.

TypeScript Integration {#typescript-integration}

TypeScript support varies significantly across frameworks, affecting type safety and developer experience. For a detailed guide on setting up TypeScript in your extension projects, check out our [TypeScript Setup Guide](./typescript-setup.md).
...
Winner: WXT edges ahead due to its auto-import system and smooth TypeScript integration out of the box. Plasmo is a close second with excellent type safety. CRXJS requires more manual setup.
...
Actual sizes vary based on dependencies, code splitting, and optimization settings. WXT and CRXJS benefit from Vite's efficient bundling, while Plasmo's additional features contribute to larger base bundles.
...
*Actual sizes vary based on dependencies and configuration.*
...
- [Chrome Extension Development with TypeScript Tutorial](./chrome-extension-development-typescript-tutorial.md)
- [TypeScript Setup for Extensions](./typescript-setup.md)
- [Chrome Extension Development Guide](./chrome-extension-development-tutorial-typescript-2026.md)
- [Architecture Patterns for Extensions](./architecture-patterns.md)
