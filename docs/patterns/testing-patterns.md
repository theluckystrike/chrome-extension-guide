...
Chrome's new headless mode (the default since Chrome 132) supports loading extensions. You can use `headless: true` with the `--headless=new` flag. For older Chrome versions, use `headless: false` and `xvfb-run` on CI to run headed mode without a display.
